...
class View(nn.Module):
    ...
    def forward(self, input):
        return input.view(-1, self.chan, self.height, self.width)

class ResDeconv(nn.Module):
    def __init__(self, block, global_avg_pool=False, z_all=None, bottleneck_shape=None):
        self.inplanes=2048
        super(ResDeconv, self).__init__()
        model = []

        if global_avg_pool:
            bottleneck_size = reduce(lambda x,y:x*y, bottleneck_shape)
            model += [nn.Linear(z_all, bottleneck_size)]
            model += [View(*bottleneck_shape)]
